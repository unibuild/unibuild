...
SET ISS_FILE_NAME=unibuild-server-win32-x64.iss
SET OUTPUT_FILE_NAME=UniBuild_server_setup_x64.exe

echo Building UniBuild server for Windows x64...
...
echo Building UniBuild server for Windows x64 completed successfully.
echo Result file: %PROJECT_DIR%\dist\%OUTPUT_FILE_NAME%

exit /B 0

:fatal
echo Building UniBuild server for Windows x64 failed.
exit /B 1