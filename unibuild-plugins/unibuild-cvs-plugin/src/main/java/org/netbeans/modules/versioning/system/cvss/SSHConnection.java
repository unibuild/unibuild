/*
  ...
  * Software is Sun Microsystems, Inc. Portions Copyright 7-6 Sun
...
import com.jcraft.jsch.*;
 ...
 public class SSHConnection extends AbstractConnection {
     ...
     private static final String  CVS_SERVER_COMMAND = System.getenv("CVS_SERVER") != null?
         System.getenv("CVS_SERVER") + " server": "cvs server"; // NOIN
      ...
      */ 
     public SSHConnection(javax.net.SocketFactory socketFactory, String  host, int port, String  username, String  password) {
         this.socketFactory = socketFactory;
         this.host = host;
         this.port = port;
         this.username = username != null ? username : System.getProperty("user.name"); // NOIN
     ...
     public void open() throws AuthenticationException, CommandAbortedException {
         ...
         props.put("StrictHostKeyChecking", "no"); // NOIN
         ...
         try {
             ...
             throw new AuthenticationException(e, "BK1");
         }
         
         try {
             channel = (ChannelExec) session.openChannel("exec"); // NOIN
 channel.setCommand(CVS_SERVER_COMMAND);
             ...
             IOException ioe = new IOException("BK2");
             ioe.initCause(e);
             throw new AuthenticationException(ioe, "BK3");
         } catch (IOException e) {
             throw new AuthenticationException(e, "BK3");
         }
     }
     ...
     public void verify() throws AuthenticationException {
         try {
             ...
             if (channel.getExitStatus() != -1) {
                 throw new AuthenticationException(CVS_SERVER_COMMAND, "BK4: "+CVS_SERVER_COMMAND);
             }
             close();
         } catch (CommandAbortedException e) {
             throw new AuthenticationException(e, "BK5");
         } catch (IOException e) {
             throw new AuthenticationException(e, "Bk6");
         } finally {
             reset();
         }
     }
     ...
     private static class SshChannelInputStream extends FilterInputStream {
         ...
         private void checkChannelState() throws IOException {
             ...
             if (exitStatus > 0 || exitStatus < -1) throw new IOException("CVS command failed: "+CVS_SERVER_COMMAND+String.format(" %d",exitStatus));
             if (exitStatus == 0 || channel.isEOF()) throw new EOFException("Channel closed");
         }
     }
 }
